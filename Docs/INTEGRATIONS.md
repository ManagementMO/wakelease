...
| Pi | Generated TS extension; `agent_start` / `agent_settled`, shutdown safety net | Uses `getSessionId()` rather than storing a session file path, and sends its own PID. Requires `agent_settled`. Eight host-SDK scenarios verified on pinned Pi 0.83.0: overlapping sessions/PID ownership, active cancellation, terminal failure, successful retry, retry exhaustion, cancellation during backoff, queued follow-up, and an idle resource reload followed by work. Retry/follow-up checks retain the same lease UUID until settlement. All network attempts are blocked and the model is mocked. The user's older 0.79.4 installation is not certified. |
