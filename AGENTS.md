...
- `XPCBoundaryTests` uses temporary anonymous listeners only. Listener pins reject before delegate admission; client pins reject incoming replies, so retain the read-only version probe and peer-identity check before mutation. Same-process test signing does not certify an Apple-issued production peer.
